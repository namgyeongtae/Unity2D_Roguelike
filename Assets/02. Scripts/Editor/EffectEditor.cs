...
public class EffectEditor : IdentifiedObjectEditor
{
    ...
    private void DrawEffectDatas()
    {
        ...
        EditorGUILayout.PropertyField(isAllowLevelExceedDatasProperty); // MaxLevel이 EffectDatas의 개수를 초과할 수 있는가?
        ...
        {
            var property = effectDatasProperty.GetArrayElementAtIndex(i);   // i 인덱스의 effectData를 가져옴

            EditorGUILayout.BeginVertical("HelpBox");
            {
                // Data의 Level과 Data 삭제를 위한 X Button을 그려주는 Foldout Title을 그려줌
                // 단, 첫번째 Data(= index 0) 지우면 안되기 때문에 X Button을 그리지 않음
                ...
                if (property.isExpanded)
                {
                    ...
                    if (stackActionsProperty.arraySize > prevStackActionsSize)
                    {
                        ...
                        // Element에서 action Property를 찾아옴
        ...
        if (GUILayout.Button("Add New Level"))
        {
            ...
            // 새 Element의 Level은 이전 Element의 Level + 1로 설정
